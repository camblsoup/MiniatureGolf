...
function GolfBall.new(world, id, x, y, audiosource)
    ...
    -- Physics
    self.body = love.physics.newBody(world, x, y, "dynamic")      -- Rigid body for the ball
    self.shape = love.physics.newCircleShape(BALL_RADIUS)         -- Circular shape
    self.fixture = love.physics.newFixture(self.body, self.shape, 1) -- Density of 1
    self.fixture:setUserData("golf_ball")
    self.fixture:setRestitution(1.0)                              -- Bounciness
    self.body:setLinearDamping(1)                                 -- Friction
...
function GolfBall:aim(client, mouse_x, mouse_y)
    local grab = false
    if self.scored or self.locked or self:isMoving()
       or (client.current_ball_id ~= 0 and client.current_ball_id ~= self.ball_id) then
        self.is_aiming = false
        return
    end
...
function GolfBall:display()
    if self.scored then return end
